# SISTEMA GESTÃO DE OLIMPIADAS 🥇🥈🥉
O Sistema de Gestão das Olimpíadas (SGO) foi criado para organizar as principais atividades dos Jogos Olímpicos. Ele permite cadastrar competições, inscrever atletas, definir locais das provas, registrar resultados e gerar relatórios de medalhas. Dessa forma, o sistema ajuda a manter o evento bem estruturado, evitando conflitos de horários e garantindo um controle eficiente das informações.


## INTEGRANTES
Natalie Santana Dias Abreu

Paloma Dias de Carvalho

## PROFESSOR
João Paulo Carneiro Aramuni

## HISTÓRIA DE USUÁRIOS 
US01 – Cadastro de Competições
  - Como organizador, quero cadastrar competições informando modalidade, data, horário, local e atletas, para que as provas sejam registradas corretamente no sistema.

US02 – Inscrição de Atletas
  - Como atleta, quero me inscrever em competições específicas, para que eu possa participar das modalidades desejadas representando meu país.

US03 – Representação por País
  - Como organizador, quero garantir que cada atleta represente apenas um país por modalidade, para que não ocorram conflitos nas inscrições.

US04 – Alocação de Locais
  - Como organizador, quero atribuir locais às competições sem conflito de horário, para que não existam sobreposições de provas em um mesmo espaço.

US05 – Registro de Resultados
  - Como juiz/árbitro, quero registrar os resultados das competições, para que o sistema determine automaticamente os atletas vencedores (1º, 2º e 3º lugares).

US06 – Relatórios de Medalhas
  - Como organizador, quero gerar relatórios com a contagem de medalhas por país, para que o desempenho de cada delegação seja acompanhado facilmente.

US07 – Consulta de Informações
  - Como usuário do sistema, quero visualizar competições, atletas inscritos e resultados, para que eu tenha acesso rápido às informações do evento.

## DIAGRAMA DE CASO DE USO
<img width="946" height="687" alt="Diagrama - Caso de Uso" src="[https://github.com/user-attachments/assets/da233e11-df65-4937-bb0d-baeb830aab52](https://github.com/Palomadcarvalho/sistema-gestao-olimpiadas/blob/main/Diagramas./01_Diagrama_Caso_Uso.jpg)" />


## DIAGRAMA DE CLASSES + PACOTES
<img width="946" height="687" alt="Diagrama - Caso de Uso - Sistema de Moeda Estudantil" src="https://github.com/user-attachments/assets/da233e11-df65-4937-bb0d-baeb830aab52" />

## DIAGRAMA DE IMPLEMENTAÇÃO 
<img width="946" height="687" alt="Diagrama - Caso de Uso - Sistema de Moeda Estudantil" src="https://github.com/user-attachments/assets/da233e11-df65-4937-bb0d-baeb830aab52" />

## DIAGRAMA DE COMPONENTES 
<img width="946" height="687" alt="Diagrama - Caso de Uso - Sistema de Moeda Estudantil" src="https://github.com/user-attachments/assets/da233e11-df65-4937-bb0d-baeb830aab52" />



